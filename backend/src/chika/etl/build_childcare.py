...
받은 원본은 지금 `data/.cache/` 에만 두고 커밋하지 않는데, MLIT 이용 목적이
막아서가 아니다 — 실제 신청 목적은 "살 곳 평가"뿐이었고, 실제 PDL1.0
라이선스는 출처 표기 조건으로 개별 시설 레코드 재배포도 허용한다(스펙
§3.1.2 정정, 2026-09-11). 커밋 대상으로 옮길지는 별도 결정 사항이다.
