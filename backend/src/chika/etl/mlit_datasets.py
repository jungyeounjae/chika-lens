...
class MlitDataset:
    ...
    #:
    #: `None` 은 **이 엔드포인트가 `_index` 를 주지 않는다**는 실측 사실이다
    #: (XPT001). 검증을 건너뛰는 것이므로 기본값을 두지 않는다 — 새 데이터셋을
    #: 추가할 때 "정체를 확인했는가"를 반드시 마주치게 한다.
    index_prefix: str | None
    #: 허용 줌. 엔드포인트마다 다르고 벗어나면 400 이다 (2026-09-08 실측):
    #: XKT025/027/029 는 z=13, XKT026 은 z=15 만, XPT001 은 z=11~15.
    zoom: int = TILE_ZOOM
...
#: 지표 13(시세)의 출처. 부동산 거래가격 정보.
#:
#: **`_index` 를 주지 않는다** — 다른 XKT 엔드포인트와 달리 응답에 정체 필드가
#: 없다. 대신 `land_type_name_ja` 등 고유한 필드 구성으로 형태를 검증한다
#: (`mlit_prices.parse_transaction`).
#:
#: `from`/`to` 분기 파라미터가 **필수**다. 없으면 400 이 온다.
TRANSACTION_PRICE = MlitDataset("XPT001", "不動産取引価格情報", None, zoom=13)

#: 지표 13 에 쓰는 거래 종별. 중고 맨션만 센다.
#:
#: 도쿄 1년치 52,553건의 구성은 中古マンション等 36,180 / 宅地(土地と建物)
#: 12,529 / 宅地(土地) 3,844 였다. 토지가 섞인 두 종별은 ㎡당 단가의 뜻이
#: 달라 (토지 지분 + 건물) 한 분포에 넣으면 중앙값이 무의미해진다. 맨션은
#: 전용면적당 단가라 역끼리 비교가 성립한다.
CONDO_LAND_TYPE = "中古マンション等"
